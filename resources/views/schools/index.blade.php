...
<div class="card-custom p-4 mb-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
            <span class="badge bg-primary-subtle text-primary px-3 py-1.5 rounded-pill fw-semibold mb-2" style="font-size: 0.78rem;">
        ...
        <div class="d-flex align-items-center gap-2">
            @php
                ...
                $count = $schools->count();
            @endphp
            <div class="px-3 py-2 bg-light border rounded-3 text-end me-2">
            ...
            @if($canAdd)
                <button class="btn btn-primary px-3 py-2 rounded-3 fw-bold shadow-sm d-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#addSchoolModal">
                    <i class="bi bi-plus-lg"></i> Tambah Unit Sekolah
                </button>
            @else
                <button class="btn btn-secondary px-3 py-2 rounded-3 fw-semibold" disabled title="Batas unit sekolah lisensi telah tercapai">
                    <i class="bi bi-lock-fill me-1"></i> Kuota Unit Penuh
                </button>
            @endif
        </div>
    </div>
    ...
    <div class="row g-3">
        @foreach($schools as $s)
            ...
            <div class="col-12 col-md-6 col-xl-4">
                <div class="card h-100 rounded-4 border {{ $isActiveContext ? 'border-primary border-2 shadow-sm' : 'border-light-subtle' }} p-3" style="background: #ffffff;">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        ...
                        @if($isActiveContext)
                            <span class="badge bg-success px-2.5 py-1 rounded-pill">
                                <i class="bi bi-check-circle-fill me-1"></i> Sedang Dikelola
                            </span>
                        @else
                            <form action="{{ route('schools.switch', $s->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="btn btn-outline-primary btn-sm rounded-pill px-3 py-0.5 fw-semibold" style="font-size: 0.78rem;">
                                    Kelola Unit Ini
                                </button>
                            </form>
                        @endif
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content rounded-4 border-0">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title fw-bold"><i class="bi bi-building-add text-primary me-2"></i>Daftarkan Unit Sekolah Baru</h5>
