    ...
    <form action="{{ route('expenses.index') }}" method="GET" class="row g-2 align-items-center">
        <div class="col-12 col-md-4">
            <div class="input-group">
                ...
                <input type="text" name="search" class="form-control bg-light border-start-0" placeholder="Cari uraian, vendor, toko..." value="{{ request('search') }}">
            </div>
        </div>
...
<div class="card-custom p-4 mb-4">
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        ...
        <div class="d-flex flex-wrap gap-2">
            ...
            <button class="btn btn-primary btn-sm rounded-3 px-3 fw-semibold d-flex align-items-center gap-1" data-bs-toggle="modal" data-bs-target="#newExpenseModal">
                    ...
                    <tr>
                        ...
                        <td>
                            @if($e->receipts->count() > 0)
                                <a href="{{ route('expenses.show', $e->id) }}" class="btn btn-sm btn-outline-secondary rounded-2">
                        ...
                        <td>
                            ...
                            <span class="badge {{ $badge['class'] }} rounded-pill px-2.5 py-1.5" style="font-size: 0.78rem;">
                                <i class="bi {{ $badge['icon'] }} me-1"></i> {{ $e->status }}
                            </span>
                        </td>
                        <td class="text-center">
                            <div class="d-flex justify-content-center align-items-center gap-1">
                                <a href="{{ route('expenses.show', $e->id) }}" class="btn btn-sm btn-outline-primary rounded-2" title="Detail & Timeline">
                                    <i class="bi bi-eye"></i>
                                </a>
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end shadow-sm border-0">
                                            ...
                                            <li>
                                                <form action="{{ route('expenses.update-status', $e->id) }}" method="POST">
                                                    ...
                                                    <button type="submit" class="dropdown-item d-flex align-items-center gap-2 text-success fw-bold">
                                            ...
                                            <li>
                                                <a href="{{ route('expenses.edit', $e->id) }}" class="dropdown-item d-flex align-items-center gap-2 text-secondary">
                                            ...
                                            <li><hr class="dropdown-divider"></li>
                                            <li>
                                                <form action="{{ route('expenses.destroy', $e->id) }}" method="POST" onsubmit="return confirm('Hapus catatan pengeluaran talangan ini?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="dropdown-item d-flex align-items-center gap-2 text-danger">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="text-center text-muted py-5">
                            ...
                            Belum ada catatan talangan pengeluaran. Klik <strong>"Input Talangan"</strong> untuk mencatat dalam &lt; 30 detik.
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow-lg">
            <div class="modal-header border-0 pb-0">
                <h5 class="modal-title fw-bold"><i class="bi bi-lightning-fill text-warning me-2"></i>Input Talangan Pribadi (&lt; 30 Detik)</h5>
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            ...
                            <input type="date" name="tanggal" class="form-control bg-light" value="{{ date('Y-m-d') }}" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold">Nominal Talangan (Rp) <span class="text-danger">*</span></label>
                            <input type="number" name="nominal" class="form-control bg-light fw-bold" placeholder="50000" min="1" step="100" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Uraian / Keperluan <span class="text-danger">*</span></label>
                        <textarea name="uraian" rows="2" class="form-control bg-light" placeholder="Contoh: Beli Kertas HVS 2 Rim & Map Raport" required></textarea>
                    </div>
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            <label class="form-label fw-semibold">Toko / Vendor</label>
                            <input type="text" name="toko_vendor" class="form-control bg-light" placeholder="Contoh: Toko ATK Sejahtera">
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold">Lokasi</label>
                            <input type="text" name="lokasi" class="form-control bg-light" placeholder="Contoh: Sukajadi">
                        </div>
                    </div>
                    <div class="mb-3">
